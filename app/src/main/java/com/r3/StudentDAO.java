...
public class StudentDAO {
    ...
    public StudentDAO(String name, String surname, String AM, String email, String password, ArrayList<String> skills, HashMap<String, ArrayList<String>> timeline) {
        /*this.name = name;
    ...
    public void createAccount() {
        ...
        allAM.addAll(accountDAO.getHashMapKeyStudent(accountDAO.accounts));
        ...
        while (!correct) {
            ...
            if (invalid == 0 && accountDAO !=null && accountDAO.accounts!=null) {
                for (int k = 0; k < accountDAO.accounts.size(); k++) {
                    if (AM.equals(accountDAO.accounts.get(allAM.get(k)).getAM())) {
        ...
        while(!coremail){
            ...
            if(accountDAO != null && accountDAO.accounts!=null) {
                for (int i = 0; i < accountDAO.accounts.size(); i++) {
                    if (email.equals(accountDAO.accounts.get(allAM.get(i)).getEmail())) {
        ...
        allAM.add(getAM());
        AccountDAO acc = new AccountDAO(getName(), getSurname(), getAM(), getEmail(), getPassword(), getSkills(), getTimeline());
    }
    ...
    public String login(String AM, String password) {
        ...
        if (accountDAO != null && accountDAO.accounts != null) {
            for (int i = 0; i < accountDAO.accounts.size(); i++) {
                if (accountDAO.accounts.containsKey(AM)) {
                    for(int j = 0; j< accountDAO.accounts.size(); j++) {

                    }
                    if (password.equals(accountDAO.accounts.get(AM).getPassword())) {
    ...
    public void editAccount(String AM) {
        ...
        switch (action) {
            case "1":
                ...
                if (newname != null) {
                    setName(newname);
                    StudentDAO temp = accountDAO.accounts.get(AM);
                    accountDAO.accounts.remove(AM);
                    AccountDAO updated = new AccountDAO(getName(),temp.getSurname(),temp.getAM(),temp.getEmail(),temp.getPassword(),temp.getSkills(),temp.getTimeline());
            ...
            case "2":
                ...
                if (newsurname != null) {
                    ...
                    StudentDAO temp = accountDAO.accounts.get(AM);
                    accountDAO.accounts.remove(AM);
                    AccountDAO updated = new AccountDAO(temp.getName(),getSurname(),temp.getAM(),temp.getEmail(),temp.getPassword(),temp.getSkills(),temp.getTimeline());
            ...
            case "3":
                ...
                if (oldpas.equals(accountDAO.accounts.get(AM).getPassword())) {
                    boolean pas = false;
                    String newpas = "";
                    boolean pas2 = false;
                    while(!pas2) {
                    ...
                    StudentDAO temp = accountDAO.accounts.get(AM);
                    accountDAO.accounts.remove(AM);
                    AccountDAO updated = new AccountDAO(temp.getName(),temp.getSurname(),temp.getAM(),temp.getEmail(),getPassword(),temp.getSkills(),temp.getTimeline());
            ...
            case "4":
                ...
                switch(decision) {
                    case "R":
                        ...
                        for(int i = 0; i< accountDAO.skillsmap.size(); i++) {
                            System.out.println(accountDAO.skillsmap.get(AM));
                        }
                        System.out.println("Please type exactly the skill you want to remove. ");
                        String removal = scanner.nextLine();
                        int sem = 0;
                        for (int i = 0; i < accountDAO.skillsmap.get(AM).size(); i++) {
                            if (removal.equals(accountDAO.skillsmap.get(AM).get(i))) {
                                accountDAO.skillsmap.remove(i);
                    ...
                    case "A":
                        ...
                        if (addition != null) {
                            accountDAO.skillsmap.get(AM).add(addition);
            ...
            case "5":
                ...
                switch (choice) {
                    case "R":
                        ...
                        System.out.println(accountDAO.timelinemap.get(AM));
                        ...
                        if ((day.equals("Monday") || day.equals("Tuesday") || day.equals("Wednesday") || day.equals("Thursday") || day.equals("Friday") || day.equals("Saturday") || day.equals("Sunday"))) {
                            ...
                            int sem = 0;
                            for (int i = 0; i < accountDAO.timelinemap.get(getAM()).get(day).size(); i++) {
                                if (hour.equals(accountDAO.timelinemap.get(getAM()).get(day).get(i))) {
                                    accountDAO.timelinemap.get(getAM()).get(day).remove(i);
                    ...
                    case "A":
                        ...
                        if ((theday.equals("Monday") || theday.equals("Tuesday") || theday.equals("Wednesday") || theday.equals("Thursday") || theday.equals("Friday") || theday.equals("Saturday") || theday.equals("Sunday")) && thehour != null) {
                            if(!accountDAO.timelinemap.get(getAM()).get(theday).contains(thehour)) {
                                accountDAO.timelinemap.get(getAM()).get(theday).add(thehour);
                                System.out.println("Available hour added successfully");
                            }else if(accountDAO.timelinemap.get(getAM()).get(theday).contains(thehour)){
    ...
    public void createTeam() {
        ...
        for(int i = 0; i< courseDAO.courses.size(); i++) {
            if(courseDAO != null && courseDAO.courses!=null) {
        ...
        if(temp_courses.contains(thecourse)) {
            ...
            int team_size = courseDAO.courses.get(thecourse);
            final ArrayList<StudentDAO> members = new ArrayList<>(team_size);
            members.add(accountDAO.accounts.get(AM));
    ...
    public void joinRequest(String AM, String message, int id,String course) {
        RequestDAO requestDAO = new RequestDAO(AM,message,id,course);
    ...
    public void viewTeams(String thecourse) {
        Scanner scanner = new Scanner(System.in);
        System.out.println("The team limit for this course is "+ courseDAO.courses.get(thecourse));
        if(teamDAO.teams.get(thecourse)!=null) {
            for (int j = 0; j < teamDAO.teams.get(thecourse).size(); j++) {
                if (teamDAO != null && teamDAO.teams != null) {
                    System.out.println(j + 1 + ". Team " + teamDAO.getHashMapKey(teamDAO.teams.get(thecourse)).get(j) + ": ");
                    for (int l = 0; l < teamDAO.teams.get(thecourse).get(teamDAO.getHashMapKey(teamDAO.teams.get(thecourse)).get(j)).size(); l++) {
                        System.out.println(teamDAO.teams.get(thecourse).get(teamDAO.getHashMapKey(teamDAO.teams.get(thecourse)).get(j)).get(l).getAM());
                    }
                }
                System.out.println("Teams's requirements: " + teamDAO.the_requirements.get(teamDAO.getHashMapKey(teamDAO.teams.get(thecourse)).get(j)) + "\n");
    ...
    public void completeEvaluation(String AM) {
        ...
        for (int i = 0; i < teamDAO.assembled.size(); i++) {
            temp_id.add(teamDAO.getHashMapKey(teamDAO.assembled).get(i));
        }
        ...
        if (!temp_courses.contains(thecourse)) {
            ...
            if (!theid.matches("[0-9][0-9][0-9][0-9]") || !temp_id.contains(Integer.parseInt(theid))) {
                ...
                if (!teamDAO.assembled.get(Integer.parseInt(theid)).contains(accountDAO.accounts.get(AM_2))) {
                    ...
                    for (int i = 0; i < teamDAO.teams.get(thecourse).get(Integer.parseInt(theid)).size(); i++) {
                        if (teamDAO.teams.get(thecourse).get(Integer.parseInt(theid)).get(i).getAM().equals(AM)) {
                            teammates++;
                            for (int j = 0; j < teamDAO.teams.get(thecourse).get(Integer.parseInt(theid)).size(); j++) {
                                if (teamDAO.teams.get(thecourse).get(Integer.parseInt(theid)).get(j).getAM().equals(AM_2)) {
                                    teammates++;
                                }
                            }
                        }
                    ...
                    if (teammates == 2) {
                        if(evaluate.eval_done.size()==0 || evaluate.eval_done.get(AM)==null || !evaluate.eval_done.get(AM).get(AM_2)) {
                            ...
                            if (evaluation < 1 || evaluation > 5) {
                                ...
                                EvaluationDAO total_evaluationDAO = new EvaluationDAO(AM,AM_2);
